...
// Google Sheets flow). An explicit mailboxId (e.g. the mailbox chosen in the
// Template Test) wins — this is how you send a placement test FROM a specific
// mailbox. Otherwise prefer any configured DB mailbox so these use the SAME
...
const resolveMailbox = async (mailboxId) => {
  try {
    if (mailboxId) {
      const chosen = await Mailbox.findById(mailboxId)
...
const sendEmail = async ({ to, subject, body, headers, mailboxId }) => {
  const mailbox = await resolveMailbox(mailboxId)
