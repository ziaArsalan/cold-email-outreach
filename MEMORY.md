...
- **2026-07-07** — **Pre-send email verification**: `emailVerificationService.js` runs free checks (format regex, MX record via `dns.resolveMx` with an in-memory per-domain cache, disposable-domain list via the `disposable-email-domains` npm package, role-based local-parts like `admin@`/`info@`) inside `campaignService.start()` — **after** `targetLeads`, **before** AI generation/enqueue, so a bad address never costs an AI call or SMTP attempt. Invalid leads get `status:'failed'` + `emailCheckStatus:'invalid'` + a human-readable `emailCheckReason`, and are excluded from that run's enqueue (never retried automatically — re-targeting requires fixing the lead or an explicit `leadIds` override). `start()` now returns `{ enqueued, skipped }`; the client alerts on skips when starting a campaign. All three checks are toggleable via `config.emailVerification` (`EMAIL_VERIFY_*` env vars, default all on). This is NOT a paid mailbox-exists verifier — MX presence only, no SMTP handshake to the recipient.
- **2026-07-07** — **Template manager UI (T-014)**: Templates tab in `App.js` with a live preview using a client-side inline `substitute()` (mirrors `templateService.render`'s `/{{\s*(\w+)\s*}}/g` — client can't import server code) and module-level `SAMPLE_VARS`. Server `DELETE /api/templates/:id` is guard-first: `Campaign.exists({ templateId })` → 400 "deactivate instead" (never orphan a running campaign's template). Campaign dropdown lists ALL templates (no active filter) — consistent with the manager; deactivate is the intended retirement path, not delete. **JSX gotcha:** literal `{{var}}` hint text must be a JS string constant (e.g. `{'Variables: {{first_name}} …'}`), never bare in JSX, or it's parsed as an expression and breaks the build.
- **2026-07-07** — **Mailbox management UI** (Dashboard tab, "Mailboxes" card): Add/Edit form + per-row Test/Edit/Pause-Resume actions, wired to the existing T-009 `/api/mailboxes*` endpoints (no new server routes). Editing a mailbox with a **blank password field keeps the existing password** (the field is intentionally never prefilled and is omitted from the PUT payload when empty). Warm-up is two fields on the Mailbox doc — `warmupEnabled` (bool) + `warmupStartDate` (date) — both must be set for the ramp to apply (`mailboxService.effectiveDailyCap`); the form only shows/sends a start-date when the warm-up checkbox is on, and clears it (`null`) when turned off. Manual Pause defaults to 24h (`minutes: 1440`), longer than the worker's own rate-limit backoff, so it doesn't silently resume mid-review.
...
- **2026-07-07** — **Outreach V2.1 batch (T-014→T-016): auto-approve plans AND bug-fixes** (user-approved after seeing the T-014 plan). Same guardrails — QA still gates shipping; a `blocked` task stops the batch.
